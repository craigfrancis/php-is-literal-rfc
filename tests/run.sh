...
echo "Test 1 / 001.phpt" | tee -a output.txt;

for V in $(printf 'a f %.0s' {1..11}); do echo -n $V >> output.txt; ../src/$V/sapi/cli/php ./001.phpt >> output.txt; done

echo "Test 2 / Laravel" | tee -a output.txt;

export SCRIPT_FILENAME="./500-laravel${SCRIPT_NAME}";
export APP_ENV="production"
export APP_DEBUG=false

for V in $(printf 'a f %.0s' {1..11}); do echo -n $V >> output.txt; ../src/$V/sapi/cgi/php-cgi "-T10" > /dev/null 2>> output.txt; done

echo "Test 3 / Symfony" | tee -a output.txt;
