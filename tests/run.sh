...
for V in $(printf 'a b %.0s' {1..11}); do echo -n $V >> output.txt ; ../src/$V/sapi/cli/php ./001.phpt >> output.txt; done
...
for V in $(printf 'a b %.0s' {1..11}); do echo -n $V >> output.txt; ../src/$V/sapi/cgi/php-cgi "-T100" > /dev/null 2>> output.txt; done
