...
Escaping is **hard** ([examples](https://github.com/craigfrancis/php-is-literal-rfc/blob/main/justification/escaping.php?ts=4)) and should be avoided (e.g. using parameterised queries), or done by well tested libraries.
...
Here are a few other [example mistakes](https://github.com/craigfrancis/php-is-literal-rfc/blob/main/justification/mistakes.php?ts=4). Notice how these insecure examples are simple edits from the Libraries' official documentation, they still "work", and are often shorter/easier than doing it correctly. I've found variations of every one of these on production websites (trust me, the shock of finding these wears off after a while, and that's why I'm here).
...
For the next step, which requires a gradual deployment (and a separate RFC to cover the details), we take the [Trusted Types](https://web.dev/trusted-types/) concept from JavaScript (which suce protects [60+ Injection Sinks](https://www.youtube.com/watch?v=po6GumtHRmU&t=92s), like innerHTML). Libraries create a stringable object as their output (which won't contain any Injection Vulnerabilities), and these get marked as "trusted" for native functions like `mysqli_query`, `preg_match`, `exec`, etc.
...
[Longer set of examples](https://github.com/craigfrancis/php-is-literal-rfc/blob/main/justification/usage.php?ts=4) and [how it can be used by libraries](https://github.com/craigfrancis/php-is-literal-rfc/blob/main/justification/example.php?ts=4) - Notice how this second script provides an `unsafe_sql` value-object to bypasses the `is_literal()` check, but doesn't actually use it (it's useful as a temporary thing, but there are much safer/better solutions). And the library just raises a warning, to let the developer know about the issue, without breaking anything.
...
Christoph Kern did a talk in 2016 about [Preventing Security Bugs through Software Design](https://www.youtube.com/watch?v=ccfEu-Jj0as) (also at [USENIX Security 2015](https://www.usenix.org/conference/usenixsecurity15/symposium-program/presentation/kern)). Starting with the premise of "Don't Blame the Developer, Blame the API", we should use well tested Libraries (written once, used by many) to ensure developers do introduce Injection Vulnerabilities. This has been achieved at Google in **Go** ([go-safe-html](https://blogtitle.github.io/go-safe-html/) and [go-safesql](https://github.com/google/go-safeweb/tree/master/safesql)) by testing for "compile time constants"; and in **Java** ([Error Prone](https://errorprone.info/), where [@CompileTimeConstant](https://errorprone.info/bugpattern/CompileTimeConstant) ensure method parameters can only use "compile-time constant expressions").

**JavaScript** is implementing [isTemplateObject](https://github.com/tc39/proposal-array-is-template-object) for "Distinguishing strings from a trusted developer from strings that may be attacker controlled" (intended to be [used with Trusted Types](https://github.com/mikewest/tc39-proposal-literals)).
...
I also agree that "SQL injection is almost a solved problem [by using] prepared statements" ([Scott Arciszewski](https://news-web.php.net/php.internals/87400)), and this is the point of `is_literal()` - so we can check that no mistakes have been made.
...
I agree with [Tyson Andre](https://news-web.php.net/php.internals/109192), please use Static Analysis.
...
[How this can be done](https://github.com/craigfrancis/php-is-literal-rfc/blob/main/justification/example.php?ts=4#L169).
...
Máté Kocsis has created a [php benchmark](https://github.com/kocsismate/php-version-benchmarks/) to replicate the old [Intel Tests](https://01.org/node/3774).
...
[Joe Watkin's implementation](https://github.com/php/php-src/compare/master...krakjoe:literals) applies the literal flags, and supports string concat at runtime.

[Dan Ackroyd's implementation](https://github.com/php/php-src/compare/master...Danack:is_literal_attempt_two) provides `literal_concat()` and `literal_implode()`.
