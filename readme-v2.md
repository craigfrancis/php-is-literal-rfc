...
But, checking the AST can get complicated for libraries; getting developers to replace their existing LiteralStrings to use Macros is unlikely (same issue as Tagged Templates); and without operator overloads ([1](https://wiki.php.net/rfc/user_defined_operator_overloads)/[2](https://wiki.php.net/rfc/userspace_operator_overloading)), concatenation would need to be handled within the macro:
